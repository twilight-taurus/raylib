...
RLAPI unsigned int rlLoadShaderCodeEx(const char *vsCode, const char *fsCode);
RLAPI unsigned int rlCompileShader(const char *shaderCode, int type);           // Compile custom shader and return shader id (type: RL_VERTEX_SHADER, RL_FRAGMENT_SHADER, RL_COMPUTE_SHADER)
RLAPI unsigned int rlLoadShaderProgram(unsigned int vShaderId, unsigned int fShaderId); // Load custom shader program
RLAPI unsigned int rlLoadShaderProgramEx(unsigned int vShaderId, unsigned int fShaderId);
...
{
    glDrawElements(GL_TRIANGLES, count, GL_UNSIGNED_INT, (unsigned int *)buffer + offset);
}
...
{
#if defined(GRAPHICS_API_OPENGL_33) || defined(GRAPHICS_API_OPENGL_ES2)
    glDrawElementsInstanced(GL_TRIANGLES, count, GL_UNSIGNED_INT, (unsigned int *)buffer + offset, instances);
#endif
}
...
unsigned int rlLoadShaderCodeEx(const char *vsCode, const char *fsCode)
{
    unsigned int id = 0;
    ...
    {
        // unload default shader loaded at startup.
//        rlUnloadShaderDefault();
        // zero the shader values
//        RLGL.State.currentShaderId = 0;
//        RLGL.State.currentShaderLocs = NULL;

        id = rlLoadShaderProgramEx(vertexShaderId, fragmentShaderId);

        // the vertex shader is not the same as the default shader
        if (vertexShaderId != RLGL.State.defaultVShaderId)
        {
            // Detach shader before deletion to make sure memory is freed
            glDetachShader(id, vertexShaderId);
        ...
        // the fragment shader is not the same as the default shader
        if (fragmentShaderId != RLGL.State.defaultFShaderId)
        {
            // Detach shader before deletion to make sure memory is freed
            glDetachShader(id, fragmentShaderId);
...
// custom shader program loading
unsigned int rlLoadShaderProgramEx(unsigned int vShaderId, unsigned int fShaderId)
{
    unsigned int program = 0;
    ...
    // NOTE: Default attribute shader locations must be binded before linking
    glBindAttribLocation(program, SHADER_LOC_VERTEX_POSITION, RL_DEFAULT_SHADER_ATTRIB_NAME_POSITION);
    glBindAttribLocation(program, SHADER_LOC_VERTEX_TEXCOORD01, RL_DEFAULT_SHADER_ATTRIB_NAME_TEXCOORD);
    glBindAttribLocation(program, SHADER_LOC_VERTEX_NORMAL, RL_DEFAULT_SHADER_ATTRIB_NAME_NORMAL);
    glBindAttribLocation(program, SHADER_LOC_VERTEX_COLOR, RL_DEFAULT_SHADER_ATTRIB_NAME_COLOR);
    glBindAttribLocation(program, SHADER_LOC_VERTEX_TANGENT, RL_DEFAULT_SHADER_ATTRIB_NAME_TANGENT);
